...
public class Main {
    public static void main(String[] args) {
        ...
        // 사이드맵
        sideMap.put(1, new MenuItems("핫봉스틱(2조각)", "핫윙 출시 30주년 기념, 더 놀라운 맛으로 돌아온 핫봉스틱!", 3500, 1));
        sideMap.put(2, new MenuItems("텐더(2조각)", "담백한 치킨 안심살로 만들어 깔끔하게 먹을 수 있는 KFC 순살 치킨스낵", 3000, 1));
        sideMap.put(3, new MenuItems("너겟(4조각)", "순 닭고기살로 만들어 부드럽고 쫄깃한 치킨너겟", 2200, 1));
            ...
            switch (choice) {
                ...
                case 1:
                    ...
                    for (int i = 1; i <= chickenMap.size(); i++) {
                        MenuItems item = chickenMap.get(i);
                        System.out.println(i + ". " + item.getMenuName() + " \t|\t "
                                + item.getMenuItemsPrice() + " \t|\t " + item.getMenuDescription());
                    }
               ...
               // <2번 버거 상품>
                case 2:
                    ...
                    for (int i = 1; i <= burgerMap.size(); i++) {
                        MenuItems item = burgerMap.get(i);
                        System.out.println(i + ". " + item.getMenuName() + " \t|\t "
                                + item.getMenuItemsPrice() + " \t|\t " + item.getMenuDescription());
                    }
